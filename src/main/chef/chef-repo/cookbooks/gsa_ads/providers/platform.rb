...
def platform_install (context)
    home_dir = node['gsa_ads']['home_dir']
    [ "#{home_dir}",
      "#{home_dir}/bin",
      "#{home_dir}/conf",
      "#{home_dir}/templates"
    ...
    iptables_info = node['gsa_ads']['iptables']
    if iptables_info['enabled'] then
        gsa_ads_iptables "platform-iptables" do
            recipe "platform"
...
def platform_install_additional_repos
    if node['platform_family'] == 'rhel' then
        ...
        epel_rpm_file_name = node['gsa_ads']['epel_repo']['file_name']
    ...
    extra_repo_platforms = node['gsa_ads']['yum_extra_repo_names']
